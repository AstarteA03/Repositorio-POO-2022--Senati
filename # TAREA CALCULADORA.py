...
class Calculadora:
    def __init__(self, numero):
        self.number = numero
        self.datos  = [0 for contador in range (numero)] #bucle si necesita 1 dato o 2 limitamos el número de datos 
    def ingresoDatos(self):
        self.datos = [int(input('Ingreso dato '+str(contador + 1)+ ' : ')) for contador in range(self.number)]
    def __del__(self):
        self.number
        self.datos
        print ('*****Tu memoria se libero******\n')
...
class Logaritmos(Calculadora):
    ...
    def logartimo(self):
        ...
        print(f'logaritmo de {a} en base {b} es: {loga}' )    
class AreaCuadrado(Calculadora):
    def __init__(self):
        Calculadora. __init__(self,2)
    def cuadrado(self):
        a,b, = self.datos
        print('Área del cuadrado es: ', a*b)
...
class AreaTriangulo(Calculadora):
    ...
    def trian(self):
        a,b, = self.datos
        print("Área del triángulo es: ", (a*b)/2)
